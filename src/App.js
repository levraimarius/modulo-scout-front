...
function App() {
  ...
  useEffect(() => {
    ...
    .then((response) => {
        ...
        !localStorage.getItem("currentScope") && localStorage.setItem("currentScope", JSON.stringify([[response.data[0].scope[0].structure.id], [response.data[0].scope[0].role.id]]))
    })
